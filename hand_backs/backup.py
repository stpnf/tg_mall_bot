...
def correct_store_name(user_input, all_known_stores, aliases_threshold=70, stores_threshold=70):
    ...
    # 1. Проверка по алиасам (сравнение со всем списком синонимов сразу)
    ...
    # 2. Fuzzy-поиск по всем названиям магазинов
...
async def perform_search(message):
    ...
    for mall_name, mall_data in MALLS_DATA[city].items():
        ...
        if matched_stores:
            results.append((mall_name, mall_data["address"], matched_stores, mall_data))

    if not results:
        await message.answer("Магазины не найдены 😔")
        return

    # 🔽 Сортировка по количеству совпадений (от большего к меньшему)
    results.sort(key=lambda x: len(x[2]), reverse=True)

    total_user_selected = len(queries)
    full_response = ""

    for mall, address, matched_stores, mall_data in results:
        ...
        text = (
            f"🏬 <b>{mall}</b> — {len(matched_stores)} / {total_user_selected} магазинов\n"
            f"<a href='{yandex_link}'>{address}</a>\n\n"
