...
def correct_store_name(user_input, all_known_stores, aliases_threshold=70, stores_threshold=80):
    ...
    # 1. Чёткое совпадение по списку магазинов
    for store in all_known_stores:
        if store.lower() == input_lower:
            return store

    # 2. Поиск по алиасам через fuzzy
    for official_name, aliases in STORE_ALIASES.items():
        match = process.extractOne(input_lower, aliases, score_cutoff=aliases_threshold)
        if match:
            return official_name

    # 3. Fuzzy-поиск по магазинам
...
async def handle_store_input(message: types.Message, state: FSMContext):
    ...
    # Сохраняем введённое и сопоставленное
    user_data[uid]["last_added"] = {
        "input": text,
        "corrected": corrected
    }
    ...
    keyboard = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(
            text="❌ Это не тот магазин",
            callback_data=f"wrong_store::{text}::{corrected}"
        )]
    ])

    # Сообщение
    response = f"<b>Магазин добавлен:</b> {corrected}\n\n"
...
async def handle_wrong_store(callback: types.CallbackQuery):
    uid = callback.from_user.id
    data = callback.data

    if data.startswith("wrong_store::"):
        parts = data.split("::")
        user_input = parts[1]
        corrected = parts[2]

        # Удаляем некорректный магазин
        if corrected in user_data[uid]["stores"]:
            user_data[uid]["stores"].remove(corrected)

        # Ищем варианты по тому, что ввёл пользователь
        similar = process.extract(user_input, ALL_STORES, limit=5)
        if not similar:
            await callback.message.edit_text("❌ Не удалось найти похожие магазины.")
            return

        buttons = [
            [types.InlineKeyboardButton(text=match[0], callback_data=f"pick_store::{match[0]}")]
            for match in similar
        ]
        keyboard = types.InlineKeyboardMarkup(inline_keyboard=buttons)

        await callback.message.edit_text(
            f"Выберите правильный магазин для: <b>{corrected}</b>",
            reply_markup=keyboard
        )

    elif data.startswith("pick_store::"):
        chosen = data.split("::")[1]
        ...
        user_data[uid]["stores"].append(chosen)


        response = f"<b>Магазин добавлен:</b> {chosen}\n\n"
        response += "<b>Текущий список:</b>\n"
        for i, store in enumerate(user_data[uid]["stores"], 1):
            response += f"{i}. {store}\n"

        await callback.message.edit_text(response)
        await callback.message.answer("Выберите действие:", reply_markup=after_store_menu())
