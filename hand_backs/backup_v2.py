    ...
    for mall_name, mall_data in MALLS_DATA[city].items():
        ...
        for store_query in queries:
            ...
            if mall_name.strip().lower() == "мега белая дача":
                ...
                if variants:
                    matched_stores.extend(variants)
                    matched_queries.add(store_query.lower())
            else:
        ...
        if matched_stores:
            results.append((mall_name, mall_data["address"], matched_stores, mall_data, len(matched_queries)))
